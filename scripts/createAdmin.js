 ...
 *
 * Usage:
 *   node scripts/createAdmin.js <email> <password> [name]  # CLI mode
 *   node scripts/createAdmin.js --serve                   # start UI mode
 *
 * If the email already has an account, it is promoted to role='admin'.
 * If there is no current admin, the first admin can be created without a setup key.
 */
...
const serveMode = args.includes('--serve') || args.length === 0;
...
function startServer() {
  const app = express();
  const port = process.env.ADMIN_UI_PORT || 5000;
  ...
  // ensure admin_replies table exists for storing admin->user messages
  db.prepare(`
...
<body>
  <div class="page">
    ...
    <form method="post">
      ...
      <p style="margin-top:12px;text-align:center"><a href="/login">Admin login</a></p>
  </div>
</body>
</html>`;
  };
...
<p class="note">Live chat integration is incoming; replies are stored here and can be used to send messages when chat is available.</p>
  ...
  app.get('/', (req, res) => {
    ...
    if (adminId) return res.redirect('/dashboard');
    res.send(renderForm());
  });
  ...
  app.post('/reply', (req, res) => {
    ...
    if (!user_id || !message) return res.redirect('/dashboard');
    // store reply
  ...
  app.listen(port, () => {
    console.log(`Create Admin UI running on http://localhost:${port}`);
    if (!process.env.ADMIN_SETUP_KEY) {
      console.log('No ADMIN_SETUP_KEY set; first admin creation is allowed without a setup key.');
    }
  });
}
...
function printUsage() {
  console.log('Usage:');
  console.log('  node scripts/createAdmin.js <email> <password> [name]    # CLI mode');
  console.log('  node scripts/createAdmin.js --serve                     # start web UI mode');
  console.log('  ADMIN_UI_PORT=5000 node scripts/createAdmin.js --serve  # specify port');
}
...
function runCli() {
  ...
  if (!password || password.length < 8) {
    console.error('Creating a new admin requires a password of at least 8 characters.');
    process.exit(1);
  }
  ...
  db.prepare(
    "INSERT INTO users (email, password_hash, name, role, email_verified) VALUES (?, ?, ?, 'admin', 1)"
  ).run(normalizeEmail(email), passwordHash, name || null);
